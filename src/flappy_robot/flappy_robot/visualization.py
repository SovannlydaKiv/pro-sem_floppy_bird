...
class Visualization(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(
            FlappyStatus, '/flappy_status', self.status_cb, 10)
    ...
    def status_cb(self, msg):
        ...
        markers.markers.append(
            self.make_marker("robot", 0, Marker.CUBE,
                             0, robot_z,
                             0.5, 0.5, 0.5,
                             0.0, 1.0, 0.0))

        # Pipes (blue cylinders)
        for i, px in enumerate(msg.pipe_positions):
            markers.markers.append(
                self.make_marker("pipes", i + 1, Marker.CYLINDER,
                                 px, 2.5,
                                 0.4, 0.4, 5.0,
                                 0.2, 0.6, 1.0))

        # Ground (brown flat cube)
        markers.markers.append(
            self.make_marker("ground", 100, Marker.CUBE,
                             0, -0.25,
                             50.0, 5.0, 0.5,
                             0.6, 0.4, 0.1))
