...
class GameLogic(Node):
    def __init__(self):
        ...
        self.sub = self.create_subscription(
            String, '/flappy_input', self.input_cb, 10)
    ...
    def input_cb(self, msg):
        if msg.data == 'flap':
            if self.game_over:
                # Restart game on flap
                self.get_logger().info("Restarting game!")
                self.reset_game()
            else:
    ...
    def game_loop(self):
        if self.game_over:
            self.get_logger().info(
                f"GAME OVER: {self.game_over_reason} | Score: {self.score} | Press SPACE to restart!",
        ...
        if self.robot_y <= 0.0:
            ...
            self.get_logger().info(f"GAME OVER: Hit the ground! Score: {self.score}")
        elif self.robot_y >= 10.0:
            self.game_over = True
            self.game_over_reason = "Flew too high"
            self.get_logger().info(f"GAME OVER: Flew too high! Score: {self.score}")
        ...
        for i in range(len(self.pipe_x)):
            if abs(self.pipe_x[i]) < 0.5:
                if (self.robot_y < self.pipe_gap_y[i] - self.gap_size/2 or
                    ...
                    self.get_logger().info(f"GAME OVER: Hit a pipe! Score: {self.score}")
